...
//声明一个信号处理函数
static void qc_signal_handler(int signo, siginfo_t *siginfo, void *ucontext); //static表示该函数只在当前文件内可见
static void qc_process_get_status(void);                                      //获取子进程的结束状态，防止单独kill子进程时子进程变成僵尸进程

//数组 ，定义本系统处理的各种信号，我们取一小部分nginx中的信号，并没有全部搬移到这里，日后若有需要根据具体情况再增加
//在实际代码中，你能想到的要处理的信号，都弄进来
qc_signal_t  signals[] = {
    // signo      signame             handler
    { SIGHUP,    "SIGHUP",           qc_signal_handler },        //终端断开信号，对于守护进程常用于reload重载配置文件通知--标识1
    { SIGINT,    "SIGINT",           qc_signal_handler },        //标识2   
	{ SIGTERM,   "SIGTERM",          qc_signal_handler },        //标识15
    { SIGCHLD,   "SIGCHLD",          qc_signal_handler },        //子进程退出时，父进程会收到这个信号--标识17
    { SIGQUIT,   "SIGQUIT",          qc_signal_handler },        //标识3
    { SIGIO,     "SIGIO",            qc_signal_handler },        //指示一个异步I/O事件【通用异步I/O信号】
...
int qc_init_signals()
{
    ...
    {        
        ...
        {   
            qc_log_error_core(QC_LOG_EMERG,errno,"sigaction(%s) failed",sig->signame); //显示到日志文件中去的 
            return -1; //有失败就直接返回
        }	
        else
        {            
            //qc_log_error_core(qc_LOG_EMERG,errno,"sigaction(%s) succed!",sig->signame);     //成功不用写日志 
            //qc_log_stderr(0,"sigaction(%s) succed!",sig->signame); //直接往屏幕上打印看看 ，不需要时可以去掉
        }
    } //end for
    return 0; //成功    
}
...
static void qc_signal_handler(int signo, siginfo_t *siginfo, void *ucontext)
{    
    //printf("来信号了\n");    
    qc_signal_t    *sig;    //自定义结构
    ...
    if(qc_process == QC_PROCESS_MASTER)      //master进程，管理进程，处理的信号一般会比较多 
    {
        //master进程的往这里走
        switch (signo)
        {
        case SIGCHLD:  //一般子进程退出会收到该信号
            qc_reap = 1;  //标记子进程状态变化，日后master主进程的for(;;)循环中可能会用到这个变量【比如重新产生一个子进程】
            break;
    ...
    else if(qc_process == QC_PROCESS_WORKER) //worker进程，具体干活的进程，处理的信号相对比较少
    {
        //worker进程的往这里走
        //......以后再增加
        //....
    ...
    else
    {
        ...
        //do nothing
    } //end if(qc_process == qc_PROCESS_MASTER)

    //这里记录一些日志信息
    //siginfo这个
    if(siginfo && siginfo->si_pid)  //si_pid = sending process ID【发送该信号的进程id】
    {
        qc_log_error_core(QC_LOG_NOTICE,0,"signal %d (%s) received from %P%s", signo, sig->signame, siginfo->si_pid, action); 
    }
    else
    {
        qc_log_error_core(QC_LOG_NOTICE,0,"signal %d (%s) received %s",signo, sig->signame, action);//没有发送该信号的进程id，所以不显示发送该信号的进程id
    }
    ...
    if (signo == SIGCHLD) 
    {
        qc_process_get_status(); //获取子进程的结束状态
    } //end if
...
static void qc_process_get_status(void)
{
    pid_t            pid;
    int              status;
    int              err;
    ...
    for ( ;; ) 
    {
        //waitpid，有人也用wait,waitpid获取子进程的终止状态，这样，子进程就不会成为僵尸进程了；
        ...
        if(pid == -1)//这表示这个waitpid调用有错误，有错误也理解返回出去，我们管不了这么多
        {
            ...
            if (err == ECHILD)         //没有子进程
            {
                qc_log_error_core(QC_LOG_INFO,err,"waitpid() failed!");
                return;
            }
            qc_log_error_core(QC_LOG_ALERT,err,"waitpid() failed!");
            return;
        }  //end if(pid == -1)
        //-------------------------------
        ...
        {
            qc_log_error_core(QC_LOG_ALERT,0,"pid = %P exited on signal %d!",pid,WTERMSIG(status)); //获取使子进程终止的信号编号
        }
        else
        {
            qc_log_error_core(QC_LOG_NOTICE,0,"pid = %P exited with code %d!",pid,WEXITSTATUS(status)); //WEXITSTATUS()获取子进程传递给exit或者_exit参数的低八位
        }
    } //end for
    return;
}
