...
static u_char *qc_sprintf_num(u_char *buf, u_char *last, uint64_t ui64,u_char zero, uintptr_t hexadecimal, uintptr_t width);
...
u_char *qc_snprintf(u_char *buf, size_t max, const char *fmt, ...)
{
    ...
    va_start(args, fmt);
    p = qc_vslprintf(buf, buf + max, fmt, args);
    va_end(args);
    return p;
}
u_char *qc_slprintf(u_char *buf, u_char *last, const char *fmt, ...)
{
    ...
    p = qc_vslprintf(buf, last, fmt, args);
    va_end(args);        //释放args   
    return p;
}
    ...
    while (*fmt && buf < last) //每次处理一个字符，处理的是  "invalid option: \"%s\",%d" 中的字符
    {
        if (*fmt == '%')  //%开头的一般都是需要被可变参数 取代的 
        {
            ...
            case 'f': //一般 用于显示double类型数据，如果要显示小数部分，则要形如 %.5f  
                ...
                //正整数部分，先显示出来
                buf = qc_sprintf_num(buf, last, ui64, zero, 0, width); //把一个数字 比如“1234567”弄到buffer中显示
                ...
                if (frac_width) //指定了显示多少位小数
                {
                    ...
                    buf = qc_sprintf_num(buf, last, frac, '0', 0, frac_width); //frac这里是小数部分，显示出来，不够的，前边填充'0'字符
                }
                fmt++;
                continue;  //重新从while开始执行
            ...
            buf = qc_sprintf_num(buf, last, ui64, zero, hex, width); 
            fmt++;
        }
...
{
    //temp[21]
    u_char      *p, temp[QC_INT64_LEN + 1];   //#define QC_INT64_LEN   (sizeof("-9223372036854775808") - 1)     = 20   ，注意这里是sizeof是包括末尾的\0，不是strlen；             
    size_t      len;
    uint32_t    ui32;
    ...
    p = temp + QC_INT64_LEN; //QC_INT64_LEN = 20,所以 p指向的是temp[20]那个位置，也就是数组最后一个元素位置

    if (hexadecimal == 0)  
    {
        if (ui64 <= (uint64_t) QC_MAX_UINT32_VALUE)   //QC_MAX_UINT32_VALUE :最大的32位无符号数：十进制是‭4294967295‬
        {
    ...
    len = (temp + QC_INT64_LEN) - p;  //得到这个数字的宽度，比如 “7654321”这个数字 ,len = 7
    ...
    while (len++ < width && buf < last)  //如果你希望显示的宽度是10个宽度【%12f】，而实际想显示的是7654321，只有7个宽度，那么这里要填充5个0进去到末尾，凑够要求的宽度
    {
        *buf++ = zero;  //填充0进去到buffer中（往末尾增加），比如你用格式  
                                          //qc_log_stderr(0, "invalid option: %10d\n", 21); 
                                          //显示的结果是：nginx: invalid option:         21  ---21前面有8个空格，这8个弄个，就是在这里添加进去的；
    }
    
    len = (temp + QC_INT64_LEN) - p; //还原这个len，也就是要显示的数字的实际宽度【因为上边这个while循环改变了len的值】
    ...
    return qc_cpymem(buf, p, len); //把最新buf返回去；
}