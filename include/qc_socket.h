...
#define QC_LISTEN_BACKLOG  511  //已完成连接队列，nginx给511，我也先按照这个来
#define QC_MAX_EVENTS      512  //epoll_wait一次最多接收这么多个事件，nginx中缺省是512

typedef struct qc_listening_s   qc_listening_t, *lpqc_listening_t;
typedef struct qc_connection_s  qc_connection_t,*lpqc_connection_t;
typedef class  CSocket           CSocket;

typedef void (CSocket::*qc_event_handler_pt)(lpqc_connection_t c); //定义成员函数指针
//和监听端口有关的结构
struct qc_listening_s
{
    int                 port;
    int                 fd;
    lpqc_connection_t   connection;
};

//以下三个结构是非常重要的三个结构，遵从官方nginx的写法；
struct qc_connection_s
{
    qc_connection_s();
    virtual ~qc_connection_s();
    //分配出去的时候初始化一些内容
    void GetOneToUse();
    //回收回来的时候做一些事情
	void PutOneToFree();
    ...
    int                      fd;                             //套接字句柄socket
	lpqc_listening_t         listening;                      //如果这个链接被分配给了一个监听套接字，那么这个里边就指向监听套接字对应的那个lpqc_listening_t的内存首地址		
	...
	qc_event_handler_pt      rhandler;                       //读事件的相关处理方法
	qc_event_handler_pt      whandler;                       //写事件的相关处理方法
	...
	//--------------------------------------------------
	lpqc_connection_t        next;                           //这是个指针，指向下一个本类型对象，用于把空闲的连接池对象串起来构成一个单向链表，方便取用
};
//消息头，引入的目的是当收到数据包时，额外记录一些内容以备将来使用
typedef struct _STRUC_MSG_HEADER
{
	lpqc_connection_t  pConn;         //记录对应的链接，注意这是个指针
...
public:	
	...
	int qc_epoll_oper_event(int fd,uint32_t eventtype,uint32_t flag,int bcaction,lpqc_connection_t pConn); 
	                                                                      //epoll操作事件
	
protected:
	//数据发送相关
	void msgSend(char *psendbuf);                                         //把数据扔到待发送对列中 
	void zdClosesocketProc(lpqc_connection_t p_Conn);                    //主动关闭一个连接时的要做些善后的处理函数	
	
private:	
	void ReadConf();                                                      //专门用于读各种配置项	
	bool qc_open_listening_sockets();                                    //监听必须的端口【支持多个端口】
	void qc_close_listening_sockets();                                   //关闭监听套接字
	bool setnonblocking(int sockfd);                                      //设置非阻塞套接字	

	//一些业务处理函数handler
	void qc_event_accept(lpqc_connection_t oldc);                       //建立新连接
	void qc_read_request_handler(lpqc_connection_t pConn);              //设置数据来时的读处理函数
	void qc_write_request_handler(lpqc_connection_t pConn);             //设置数据发送时的写处理函数
	void qc_close_connection(lpqc_connection_t pConn);                  //通用连接关闭函数，资源用这个函数释放【因为这里涉及到好几个要释放的资源，所以写成函数】

	ssize_t recvproc(lpqc_connection_t pConn,char *buff,ssize_t buflen); //接收从客户端来的数据专用函数
	void qc_wait_request_handler_proc_p1(lpqc_connection_t pConn,bool &isflood); 
	                                                                      //包头收完整后的处理，我们称为包处理阶段1：写成函数，方便复用      
	void qc_wait_request_handler_proc_plast(lpqc_connection_t pConn,bool &isflood);   
	...
	lpqc_connection_t qc_get_connection(int isock);                     //从连接池中获取一个空闲连接
	void qc_free_connection(lpqc_connection_t pConn);                   //归还参数pConn所代表的连接到到连接池中	
	void inRecyConnectQueue(lpqc_connection_t pConn);                    //将要回收的连接放到一个队列中来
	
	//和时间相关的函数
	void    AddToTimerQueue(lpqc_connection_t pConn);                    //设置踢出时钟(向map表中增加内容)
	...
	void DeleteFromTimerQueue(lpqc_connection_t pConn);                  //把指定用户tcp连接从timer表中抠出去
	void clearAllFromTimerQueue();                                        //清理时间队列中所有内容

	//和网络安全有关
	bool TestFlood(lpqc_connection_t pConn);                             //测试是否flood攻击成立，成立则返回true，否则返回false
...
private:
	...
	//和连接池有关的
	std::list<lpqc_connection_t>  m_connectionList;                      //连接列表【连接池】
	std::list<lpqc_connection_t>  m_freeconnectionList;                  //空闲连接列表【这里边装的全是空闲的连接】
	...
	std::list<lpqc_connection_t>  m_recyconnectionList;                  //将要释放的连接放这里
	...
	std::vector<lpqc_listening_t> m_ListenSocketList;                    //监听套接字队列
	struct epoll_event             m_events[QC_MAX_EVENTS];              //用于在epoll_wait()中承载返回的所发生的事件
